...
# Written in 2014. Modernised 2024.
...
def car_wear(car: dict) -> float:
    ...
    Falls back to the car's own odometer when 'last_service_km' is absent
    (i.e. freshly serviced, 0 % worn) so the report never crashes on missing data.
    """
...
def fleet_summary(fleet: list[dict]) -> dict:
    """Return a summary dict with car count, how many are due, and average wear %.

    Uses float division for the average so fractional percentages are preserved.
    """
    total = 0.0
    due = 0
    for car in fleet:
...
def print_report(fleet: list[dict]) -> None:
    """Print the nightly fleet-health report to stdout and append to the log file."""
